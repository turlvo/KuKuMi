...
(function(_w) {
    ...
    _w.showLoading = function() {
        ...
        _body.appendChild(_methods[defaultOptions.name]());

        defaultOptions.callback();
    };
    _w.hideLoading = function() {
        var _mask = document.getElementById('_mask');
        var _loading = document.getElementById('_loading');
        if (typeof _mask !== 'undefined' && _mask === null && typeof _loading !== 'undefined' && _loading === null) {
            return;
        }
        _mask.remove();
        _loading.remove();
